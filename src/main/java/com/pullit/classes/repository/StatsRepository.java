        ...
        WITH latest_attempts AS (
         ...
         exam_rankings AS (
             SELECT
                 ...
                 user_id,
                 score,
                 RANK() OVER (PARTITION BY exam_id ORDER BY score DESC) as rank_position,
                 COUNT(*) OVER (PARTITION BY exam_id) as total_students,
                 ROUND(
                         PERCENT_RANK() OVER (PARTITION BY exam_id ORDER BY score ASC) * 100,
                         1
                 ) as percentile,
                 NTILE(4) OVER (PARTITION BY exam_id ORDER BY score ASC) as quartile
             FROM exam_scores
         )
        SELECT
            er.exam_id,
            ...
            ROUND(
                    (er.rank_position - 1) / er.total_students * 100,
                    1
            ) as top_percentage
